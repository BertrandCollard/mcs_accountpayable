...
/*
 ...
 MODIFIED   (MM/DD/YY)
 vismishr    05/07/15 - added back getCookiesForURL: for MAF backward
                        compatibility
 vismishr    04/22/14 - ER 20321982
 vismishr    03/16/14 - getCookie: API for OWSM mobile agent
...
/**
 * @deprecated
 * Returns cookies for a given URL in format that OWSM mobile agent can directly
 * inject in the REST calls.
 * @param URL NSString representing the URL for which cookies are required.
 * @return NSString all the cookies in format CookieName1=CookieValue1,....
 */
- (NSString *)getCookiesForURL:(NSString *)theURL;

/**
 * Returns cookies for a given URL in format that OWSM mobile agent can directly
 * inject in the REST calls.
 * @param URL NSString representing the URL for which cookies are required.
 * @param includeHeaders BOOL to specify whether to sent custom headers or not
 * @return NSDictionary contraining all the cookies in format
 * CookieName1=CookieValue1,.... and custom headers if requested
 */
- (NSDictionary *)getRequestParametersForURL:(NSString *)theURL
